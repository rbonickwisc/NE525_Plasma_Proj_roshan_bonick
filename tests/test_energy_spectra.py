...
from tokamak_source_model.energy_spectra import DT_E0_EV, EnergySpectrumParameters, dt_ballabio_fwhm_eV, fwhm_to_sigma_eV, sample_birth_energies_from_model_eV
...
def test_dt_ballabio_fwhm_matches_177_sqrt_T_formula():
    T_i_keV = np.array([1.0, 4.0, 9.0])
    fwhm_eV = dt_ballabio_fwhm_eV(T_i_keV)

    expected_eV = 1.0e3 * 177 * np.sqrt(T_i_keV)
