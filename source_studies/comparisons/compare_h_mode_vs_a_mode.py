...
from tokamak_source_model.utils.case_builder import build_default_mesh, build_a_mode_paper_model, build_generic_pedestal_model
...
def main() -> None:
    ...
    relative_rate_percent = 100.0 * (
        a_mode["total_rate_n_per_s"] - h_mode["total_rate_n_per_s"]
    ) / h_mode["total_rate_n_per_s"]

    print("H-mode vs A-mode comparison")
    print("-" * 40)
    ...
    fig.suptitle("H-mode vs A-mode source comparison (Moving from plasma center to plasma edge)")
    fig.tight_layout()
    fig.savefig(output_dir / "h_mode_vs_a_mode_comparison.png", dpi=220)
    plt.close(fig)
